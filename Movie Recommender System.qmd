---
title: "Movie Recommender System.qmd"
self-contained: true
format: html
editor: visual
---
...
```{r}
movie_data <- read.csv("/Users/phamlethuylinh/Desktop/IMDB-Dataset/movies.csv")
rating_data <- read.csv("/Users/phamlethuylinh/Desktop/IMDB-Dataset/ratings.csv")
str(movie_data)
```
...
```{r}
rm(genre_mat1, genre_mat2, movie_genre, movie_genre2)
```
...
```{r}
SearchMatrix <- cbind(movie_data[,1:2], genre_mat2[]) #only choose genre from 1 to not including 2, which is genre
head(SearchMatrix) 
```
```{r}
#converting to a  matrix using recommenderlab
...
```{r}
#filter out all films that have views and number of votes under 50
...
```{r}
op_recommendations <- 10 # the number of items to recommend to each user
